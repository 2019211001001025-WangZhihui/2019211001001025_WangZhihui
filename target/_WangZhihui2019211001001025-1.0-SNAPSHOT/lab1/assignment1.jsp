...
<%-- todo 3 : use c:forEach to print 2,4,6,8,10   --%>
        <ul>
             <c:forEach var="i" begin="2" end="11" step="2">
                <li>$${i}</li>
            </c:forEach>
        </ul>
...
%>
    ...
    <h1>c:forEach to print all elements of array</h1>
        <ul>
            <c:forEach var="i" items="${pageScope.words}">
                <li>${i}</li>
            </c:forEach>
        </ul>
    ...
    <h1>c:forEach to print "one","three","five","seven","nine"  of array</h1>
        <ul>
            <c:forEach var="i" items="${pageScope.words}" step="2">
                <li>${i}</li>
            </c:forEach>
        </ul>
<%
...
// todo 6 : add a new person into PersonList - person name and dog name
    personList.add(new Person("Amy",new Dog("Jessie")));
...
<%--todo 8 : use c:forToken to print all words --%>
    <h2>c:forToken to print all words</h2>
        <ul>
            <c:forTokens delims="-:" items="${str}" var="str">
                <li>${str}</li>
            </c:forTokens>
        </ul>
</body>
